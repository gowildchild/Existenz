# ==========================================================================
# EXISTENZ master/struct/module/cliStateInit.py 19:40 build
            ...
            if token in ["SignaturesPy", "SignaturesJson"] or filename == "existentialCores.json":
                ...
                try:
                    ...
                    for k, d in schema_data.get("existentialCore", {}).items():
                        ...
                        line_entry = f'    "{k}":'.ljust(33) + f'{{ "val": {val_string}"expr": "{calculated_expr}",'.ljust(22) + f'"type": "{struct_type}", "comment": "{d.get("comment", "").replace('"', '\\"')}" }}'
                    ...
                    try:
                        ...
                        with open(target_path, "w", encoding="utf-8") as f:
                            ...
                            for k, v in schema_data.get("existentialCoreThreatLegal", {}).items():
                                target_node = next((d["threat"] for d in schema_data["existentialCore"].values() if isinstance(d, dict) and "threat" in d and str(d["val"]) == k), None)
