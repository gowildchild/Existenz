...
def verify_structural_hashes() -> bool:
    ...
    # RULE 1: The Short Sign must be the EXACT front-end slice of the Long Signature
    if not existentialCoreCheckSignature.startswith(short_sign_hex):
        print("  [-] CRITICAL ALERT: Architectural misalignment inside existentialCoreCheck.py!")
        print(f"      config Sign parameter (Front) : {short_sign_hex}")
        print(f"      config Signature whole string : {existentialCoreCheckSignature}")
        print("      [!] Error: The Sign integer must match the literal front of the Signature string.")
        return False
        
    # RULE 2: The calculated live master token must match that exact front slice
    if hmac.compare_digest(check_structures_sign, short_sign_hex):
        print(f"  [+] Passed: Unified sequence signature string verified clean (Front: 0x{check_structures_sign}).")
    else:
        print("  [-] CRITICAL ALERT: Live layout tracking chain token drift exposed!")
        print(f"      Expected Prefix inside file  : {short_sign_hex}")
